  ...
  return createTool({
    ...
    execute: async ({ context: { query } }) => {
      try {
        ...
        console.log(`Found ${formattedResults.length} relevant results for query: "${query}"`);

        return {
          context: formattedResults.map(
            (doc) => `- ${doc.text} (Similarity: ${doc.similarity.toFixed(2)})`
          ).join('\n'),
          query,
